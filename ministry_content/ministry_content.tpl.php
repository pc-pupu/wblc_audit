...
inner join file_managed as fm on t.field_minister_image_fid= fm.fid order by t.nid desc limit 6
");
?>
...
?>
<li><h3 style="font-size:12px;text-transform:uppercase;"><?php echo $record->title; ?></h3>
 <?php 
 if($temp == 5)
 ...
 if($temp == 2)
    $linkPath = "search-establishment-information";
 if($temp == 1)    
	$linkPath = "applicant-login";
 if($temp == 0)
    $linkPath = "user-manual";
   ...
   // echo l(t('<img src="'.base_path(). 'sites/default/files/Minister/'.$record->filename.'" width="270" height="200">'),$linkPath, array('html' => TRUE,'attributes'=>array())); 
   ?>
